...
| BL-075 | UX: Replace visible edit/delete actions on contact rows with 3-dot overflow menu | 🟡 Medium | 📋 Backlog | — | Improvement | Agent 1 | In Contacts, each contact currently shows direct edit/delete action buttons, cluttering the UI. Replace with a 3-dot (⋯) overflow menu button. On click, show a small menu with two items: "עריכה" (Edit) and "מחיקה" (Delete). Implementation: (1) Find the contact list/card component. (2) Replace visible edit/delete buttons with a 3-dot overflow button. (3) Show menu on click with Edit and Delete items. (4) Delete must still use existing confirmation flow. (5) Keep existing edit/delete logic unchanged. (6) Menu works in Hebrew/RTL and mobile. (7) Accessibility: aria-label on 3-dot button; menu items keyboard accessible; close on outside click or Escape key. Do NOT change contact data logic. Minimal UI change only. | 3-dot button visible on each contact row; clicking opens menu with Edit and Delete; Delete triggers existing confirmation flow; menu accessible via keyboard; RTL-correct; build+lint pass |
| BL-076 | Improvement: CSV import template missing Hebrew birthday date support | 🟡 Medium | 📋 Backlog | — | Improvement | Agent 1 | The downloadable CSV template does not include a field for Hebrew birthday. Add optional Hebrew birthday columns: Hebrew Birthday Day, Hebrew Birthday Month, Hebrew Birthday Year (optional). Keep the existing regular Birthday column unchanged. Import logic should map these fields to the existing `hebrewBirthday` contact field. Hebrew format instructions should be added to the template. Must not break existing CSV imports. | Hebrew birthday columns present in template; fields are optional; import maps to hebrewBirthday; existing Gregorian birthday imports unaffected; Hebrew format instructions in template; build+lint pass |
